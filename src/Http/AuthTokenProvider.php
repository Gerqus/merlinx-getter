	...
	private function refreshToken(bool $force): string
	{
		return $this->lock->withLock($this->refreshLockKey, function () use ($force): string {
			...
			try {
				...
				throw new HttpRequestException('MerlinX token request failed.', null, null, $e);
			}

			if ($status >= 400) {
				throw new HttpRequestException('MerlinX token request failed with status ' . $status . '.', $status, $body);
			}
